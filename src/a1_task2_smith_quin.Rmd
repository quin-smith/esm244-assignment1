...
The following principal components analysis uses the `state` dataset, which is included within the `datasets` package that comes pre-installed with R.  It contains data of facts and figures relating to the 50 states.  This analysis includes data from `state.x77` and `state.abb` dataframes including the following variables:
